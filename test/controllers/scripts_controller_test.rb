...
class ScriptsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create script" do
    assert_difference('Script.count') do
      post scripts_url, params: { script: { context: @script.context, description: @script.description, name: @script.name, owningApp: @script.owningApp, status: @script.status, user_id: @script.user_id, version: @script.version } }
    end
  ...
  test "should update script" do
    patch script_url(@script), params: { script: { context: @script.context, description: @script.description, name: @script.name, owningApp: @script.owningApp, status: @script.status, user_id: @script.user_id, version: @script.version } }
